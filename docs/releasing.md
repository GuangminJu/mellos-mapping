...
安装前需要 Node.js 18.17 及以上的 18.x，或 20.3 及以上版本、对应宿主 CLI，以及
该宿主正常工作的账户/环境。支持 Windows 10+、macOS 13.0+ 和 glibc 2.28+ 的 Linux，
均支持 x64 与 arm64；系统还须满足所选 Node.js 版本的更高要求。
发行包自带原生锁绑定，无需用户编译。
...
源码开发使用 Node.js 22.12+（成品要求 `^18.17.0 || >=20.3.0`）。在主源码的开发分支
修改和验证，避免直接编辑生成的宿主分支。
