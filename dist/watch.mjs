...
import { existsSync as existsSync6, readFileSync as readFileSync4, realpathSync as realpathSync4 } from "node:fs";
import { basename as basename3, dirname as dirname10, join as join9 } from "node:path";
var STAR_URL = "https://github.com/GuangminJu/mellos-mapping";
var DAY_MS = 864e5;
function isNpmInstallation(entry) {
  try {
    const root = dirname10(dirname10(realpathSync4(entry)));
    if (basename3(dirname10(root)).toLowerCase() !== "node_modules" || existsSync6(join9(root, ".git")) || existsSync6(join9(root, ".codex-plugin")) || existsSync6(join9(root, ".claude-plugin"))) return false;
    const pkg = JSON.parse(readFileSync4(join9(root, "package.json"), "utf8"));
...
var reminder = createStarReminder({ entry: fileURLToPath2(import.meta.url), userBase: homedir2(), env: process.env });
