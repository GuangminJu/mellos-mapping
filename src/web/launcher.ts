...
import { LedgerError, STORE_LOCK_PROTOCOL } from '../store/transaction.js';
...
interface WebHealth { readonly file: string; readonly surfaces?: string[]; readonly formats?: number[]; readonly lockProtocol?: string }
async function runningWebRuntime(defaultFile: string): Promise<{ url: string; health: WebHealth } | undefined> {
  try {
    ...
    if (response.ok) {
      const health = await response.json() as WebHealth;
...
export async function runningWebUrl(defaultFile: string): Promise<string | undefined> {
  // Stopping an old service must remain possible regardless of its protocol.
...
export async function openWebPreview(defaultFile: string, entry: string, page?: string, terminal = false): Promise<string> {
  ...
  if (runtime) {
    const { url, health } = runtime;
    if (health.lockProtocol === STORE_LOCK_PROTOCOL &&
        (!health.formats?.includes(2) || (terminal && !health.surfaces?.includes('web-terminal')))) {
  ...
  // A service discovered during startup may be an older process that won the
  // race or recovered after the first health request. Check every return path.
