 ...
 */
export function isStorePath(relativePath) {
    if (relativePath === DEFAULT_FILE_NAME || relativePath === PAGES_DIR_NAME)
        return true;
    ...
    return class MmapGateway extends _classSuper {
        ...
        async read(sessionId) {
            ...
            const pages = await Promise.all(listPageFiles(defaultFile).map(async (file) => {
                ...
                return loaded.ok
                    ? { page, map: loaded.value, error: null, mtimeMs }
        ...
        async ensureWatch(cwd, defaultFile) {
            if (!this.spec.watch || this.closed)
                return;
            ...
            const entry = { watcher, timer: undefined };
            watcher.on('all', (_event, path) => {
                if (this.closed || !isStorePath(relative(storeDir, path)))
                    return;
                if (entry.timer !== undefined)
                    clearTimeout(entry.timer);
