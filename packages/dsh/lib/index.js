...
import { PAGES_DIR_NAME, STATE_FILE_RELATIVE_PATH, describeStoreError, listPageFiles, loadMapFile, migrateLegacyStore, pageFilePath, pageIdOfFile } from "mellos-mapping/store";
...
let MmapGateway = (() => {
	...
	return class MmapGateway extends _classSuper {
		static {
		...
		*/
		async read(sessionId) {
			...
			const pages = await Promise.all(listPageFiles(defaultFile).map(async (file) => {
		...
		*/
		async resolveCwd(sessionId) {
			const header = this.ctx.sessions.list().find((session) => session.header.id === sessionId)?.header ?? await this.coldHeader(sessionId);
			if (header?.cwd === void 0) return void 0;
			try {
				const canonical = await realpath(header.cwd);
				return (await stat(canonical)).isDirectory() ? canonical : void 0;
			} catch {
				return;
			}
		}
		...
		*/
		async ensureWatch(cwd, defaultFile) {
			...
			while (this.watched.size >= this.spec.maxWatchedDirs) {
				const [oldestCwd, oldest] = this.watched.entries().next().value;
				this.watched.delete(oldestCwd);
				if (oldest.timer !== void 0) clearTimeout(oldest.timer);
...
//#endregion
export { MmapGateway, MmapGateway as default, STATE_FILE_RELATIVE_PATH, isStorePath, pageFilePath, resolveSpec };
