 ...
 * semantic obligation.
 */
import { aggregateMap, flipForSequence, isNeutralKind, zoomMode } from 'mellos-mapping/semantics';
 ...
 */
export function stepForScale(scale, current) {
    let raw = -4;
...
export function estimateTextWidth(text) {
    ...
    for (const ch of text)
        width += (ch.codePointAt(0) ?? 0) > 0xff ? 13 : 7.2;
    return width;
}
/** Hard word-wrap by character budget (CJK counts double), for in-box notes. */
export function wrapChars(text, budget) {
    const out = [];
    let line = '';
    let cost = 0;
    for (const ch of text) {
...
function boxLines(node, zoom, neutral) {
    ...
    const glyph = neutral
        ? (node.kind !== undefined ? kindGlyph(node.kind, true) : undefined)
        : STATUS_GLYPHS[node.status];
    const label = glyph !== undefined ? `${glyph} ${node.label}` : node.label;
    const lines = [{ text: label, role: 'label' }];
    if (zoomMode(zoom) !== 'detail')
        return lines;
    const budget = zoom >= 2 ? DETAIL_PLUS_BUDGET : DETAIL_BUDGET;
    if (!neutral && node.evidence !== undefined) {
        for (const text of wrapChars(node.evidence, budget.chars).slice(0, 2))
            lines.push({ text, role: 'evidence' });
    }
    if (node.detail !== undefined) {
        for (const text of wrapChars(node.detail, budget.chars).slice(0, budget.noteLines))
            lines.push({ text, role: 'note' });
    }
    return lines;
}
/**
 ...
 */
export function computeLayout(map, zoom = 0) {
    ...
    const regionOf = (node) => node.lane !== undefined ? laneIndexOf.get(node.lane) ?? regions - 1 : regions - 1;
    const laneX = [];
    const laneW = [];
    if (laneCount > 0) {
    ...
    for (const band of bands) {
        ...
        for (const { node, spec } of speced) {
            ...
            const box = { node, x, w: spec.w, h: spec.h, lines: spec.lines, y: 0 };
            row.push(box);
            placed.push(box);
    ...
    for (const header of laneHeaders)
        width = Math.max(width, header.x + header.w + MARGIN);
    ...
    for (const edge of working.edges) {
        ...
        if (si === undefined || ti === undefined || ti <= si)
            continue;
        routed.push({
            from: edge.from, to: edge.to, a, b, label: edge.label,
    ...
    for (const wire of bent) {
        ...
        for (let d = 0; chosen === undefined && d * THREAD_STEP <= hi - lo; d++) {
            for (const c of d === 0 ? [wire.ex] : [wire.ex - d * THREAD_STEP, wire.ex + d * THREAD_STEP]) {
                if (c < lo || c > hi)
                    continue;
                if (descents.some(taken => Math.abs(taken - c) < THREAD_SEP))
                    continue;
                let hit = false;
                for (let band = wire.si + 1; band < wire.ti && !hit; band++)
                    hit = blockedByBox(band, c);
                if (!hit) {
                    chosen = c;
                    break;
                }
        ...
        if (chosen === undefined)
            chosen = width - MARGIN + THREAD_FALLBACK_PAD + fallbackCount++ * THREAD_SEP;
    ...
    for (const wire of bent) {
        if (wire.cx === undefined) {
            const landing = { wire, gap: wire.si, lo: Math.min(wire.sx, wire.ex), hi: Math.max(wire.sx, wire.ex), row: 0 };
            runs.push(landing);
            runsOf.set(wire, { landing });
        }
        else {
            const entry = { wire, gap: wire.si, lo: Math.min(wire.sx, wire.cx), hi: Math.max(wire.sx, wire.cx), row: 0 };
            const landing = { wire, gap: wire.ti - 1, lo: Math.min(wire.cx, wire.ex), hi: Math.max(wire.cx, wire.ex), row: 0 };
    ...
    for (let gap = 0; gap < gapRows.length; gap++) {
        const rowEnds = []; // rightmost occupied x per packed row
    ...
    const boxes = placed;
    const trackY = (run) => (gapTop[run.gap] ?? 0) + GAP_BREATHE_TOP + (run.row + 0.5) * TRACK_H;
    // -- emit the polylines --
    const edges = [];
    for (const wire of routed) {
        ...
        if (wireRuns === undefined)
            continue; // unreachable: every bent wire packed its runs
        if (wire.cx === undefined) {
