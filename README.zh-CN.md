...
服务器）、`dist/watch.mjs`（面板），以及 `dist/store-paths.mjs`（存储的
路径词汇；纯 node 的面板启动脚本从这里导入，而不是自己抄一份文件名）。
...
| --- | --- | --- |
| `·` | `.` | planned——已声明，未开工 |
| `⠿`（转圈） | `*`（转圈） | in-progress；`⠿` 是 spinner 静止时的样子，给不能做动画的表面用 |
| `■` | `#` | done，且有证据 |
...
| `.mellos/*.tmp` | 正在落盘的一次写入——绝不会留存；留下来就说明那次写入失败并已被报告 |
